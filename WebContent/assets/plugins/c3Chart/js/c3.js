(function (window) {
    ...
    c3.generate = function (config) {
        ...
        var __legend_show = getConfig(['legend', 'show'], true),
            __legend_item_width = getConfig(['legend', 'item', 'width'], 80), // auto
        ...
        function getDefaultPaddingWithAxisId() {
            return 40; // calc automatically
        }
        function getParentWidth() {
            return +d3.select(__bindto).style("width").replace('px', ''); // if rotated, use height
        }
        function getParentHeight() {
            return +d3.select(__bindto).style('height').replace('px', ''); // if rotated, use width
        }
        ...
        */
        ...
        function lineWithRegions(d, x, y, _regions) {
            ...
            for (i = 0; i < d.length; i++) {
                ...
                // Draw with region // Fix for horizotal charts
                else {
        ...
        function init(data) {
            ...
            // set names if names not specified
            ...
            if (__zoom_enabled || __scroll_enabled) { // __zoom_privileged here?
        ...
        function selectShape(target, d, i) {
            ...
            if (__data_selection_grouped || isWithin) {
                ...
                __point_onclick(d, _this); // should be __data_onclick
            }
        }
        ...
        function drag(mouse) {
            ...
            // binary search when multiple xs
            main.selectAll('.-shapes').selectAll('.-shape')
                ...
                .each(function (d, i) {
                    ...
                    if (isWithin ^ isIncluded) {
                        ...
                        // included/unincluded callback here
                        _this.classed(SELECTED, !isSelected);
                        toggle(!isSelected, _this, d, i);
                    }
                });
